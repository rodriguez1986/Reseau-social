...
class CommentConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        user, post, comment = await self.save_comment(username, comment_text)
        comment_count = await self.get_comment_count()

        # Envoyer le commentaire à tous les clients
        ...
        # Envoyer notification uniquement si ce n'est pas l'auteur lui-même
        if post.author != user:
            await self.channel_layer.group_send(
    ...
    def save_comment(self, username, comment_text):
        ...
        # Créer une notification si ce n’est pas l’auteur lui-même
        if post.author != user:
